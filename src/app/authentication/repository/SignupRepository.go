...
func NewCreateUserRepository(gDB *gorm.DB) *createUserRepository {
	return &createUserRepository{gDB: gDB}
}
...
func (user *createUserRepository) CreateNewUser(ctx context.Context, bffCreateUserRequest models.BFFCreateUserRequest) error {
	...
	result := user.gDB.WithContext(ctx).Table(constants.UsersTableName).Create(&NewUser)
	if result.Error != nil {
