...
func (service *WatchlistService) ServiceWatchlist(ctx context.Context, spanCtx context.Context, bffAdgToWatchlistRequest models.BFFAdgToWatchlistRequest, username string) (error, []models.WatchlistWithID, []string) {
	postgresClinet := utils.GetPostgresClient()
	tx := postgresClinet.GormDB
	var watchlistsWithId []models.WatchlistWithID
	var warnings []string

	userId, err := service.watchlistRepository.GetUserIdByUsername(ctx, tx, username)
	...
	case models.GET:
		watchlistsWithId, err = service.watchlistRepository.GetWatchlistsWithId(ctx, tx, *userId, bffAdgToWatchlistRequest.ScripId)
		if err != nil {
			return err, nil, nil
		}
	...
	case models.DEL:
		validWatchlistIds, err := service.watchlistRepository.DeleteScripsFromWatchlists(ctx, tx, *userId, bffAdgToWatchlistRequest.WatchlistIds, bffAdgToWatchlistRequest.ScripId)
		if err != nil {
			return err, nil, nil
		}
	...
	case models.ADD:
		addedWatchlistIds, skippedWatchlistIds, limitExceededWatchlistIds, err := service.watchlistRepository.AddScripsToWatchlists(ctx, tx, *userId, bffAdgToWatchlistRequest.WatchlistIds, bffAdgToWatchlistRequest.ScripId)
		if err != nil {
			return err, nil, nil
		}
